...
type Props = {
  ...
  especie: string; onEspecie: (v: string) => void;
  sexo: string; onSexo: (v: string) => void;
...
function MenuSelect({
  ...
  return (
    <div className="mselect" ref={boxRef}>
      ...
      <style jsx>{`
        .mselect { position: relative; width: 100%; max-width: 260px; }
        .mselect-trigger{
          width:100%; display:flex; align-items:center; justify-content:space-between;
          padding:10px 12px; border:1px solid var(--line); border-radius:12px;
          background:#fff; box-shadow:0 4px 14px rgba(43,27,18,.06);
          color:#2b1b12; font-weight:400;
        }
        .chev{ color:#8B4513; opacity:.9; }
        .mselect-menu{
          position:absolute; left:0; top:calc(100% + 8px); z-index:40;
          width:100%; min-width:220px; background:#FFF1E6; border-radius:10px;
          box-shadow:0 10px 24px rgba(0,0,0,.18); overflow:hidden;
        }
        .mselect-item{
          width:100%; text-align:left; padding:10px 14px; color:#8B4513; font-weight:400;
  ...
  return (
    <section className="filters">
      ...
      <style jsx>{`
        ...
        .search input:focus { border-color:#d7c4b2; }
        .searchIcon {
          position:absolute; left:10px; top:50%; transform:translateY(-50%);
          color:#a78d7b; pointer-events:none; display:flex; align-items:center;
        }
