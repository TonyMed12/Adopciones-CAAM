...
function getFotoSrc(m: Partial<Mascota>) {
    return (m as any).foto || (m as any).fotoUrl || (m as any).imagen || (m as any).image || (m as any).img || null;
}
            ...
            <table className="min-w-[950px] w-full border border-slate-200 rounded-lg overflow-hidden">
                ...
                <tbody className="divide-y divide-slate-100">
                    {data.map((m) => {
                        ...
                        // Normalizar sexo
                        const sexo =
                            m.sexo?.toLowerCase() === "h" || m.sexo?.toLowerCase() === "hembra" ? "Hembra" : "Macho";

                        const colorSexo = sexo === "Hembra" ? "bg-pink-100 text-pink-700" : "bg-blue-100 text-blue-700";
