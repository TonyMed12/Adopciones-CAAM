    ...
    <div className={"mselect " + widthClass} ref={boxRef}>
      ...
      <style jsx>{`
        ...
        .mselect-trigger{
          ...
          background:#fff; box-shadow:0 4px 14px rgba(43,27,18,.06);
          color:#2b1b12; font-weight:400; /* sin negritas */
        }
        .chev{ color:#8B4513; opacity:.9; }
        .mselect-menu{
          position:absolute; left:0; top:calc(100% + 8px); z-index:40;
          width:100%; background:#FFF1E6; border-radius:10px;
          box-shadow:0 10px 24px rgba(0,0,0,.18); overflow:hidden;
        }
        .mselect-item{
          width:100%; text-align:left; padding:10px 14px; color:#8B4513;
          font-weight:400; /* sin negritas */
          background:transparent; border:none; cursor:pointer;
          transition:background .12s ease;
        }
        .mselect-item:hover{ background:#FDE68A; }
        .mselect-item.is-active{ background:#FDE68A; }
      `}</style>
    </div>
  );
}
...
export default function FormMascota({
  ...
  function handleSubmit(e: React.FormEvent) {
    e.preventDefault();
    const nueva: Partial<Mascota> = {
      nombre,
      especie,
      raza,
      descripcion,
      edadMeses: Number.isFinite(edadMeses) ? Math.max(0, Math.floor(edadMeses)) : 0,
      sexo: sexo as any,
      tamano: tamano as any,
      // 👇 añadimos la foto como dataURL (ajusta el nombre de campo si tu tipo Mascota usa otro)
      foto: fotoPreview || undefined,
    };
    onSubmit(nueva as Mascota);
  }
    ...
    <form onSubmit={handleSubmit} className="form">
      ...
      <style jsx>{`
        ...
        label {
          ...
          color: var(--text-muted, #7a5c49); text-transform: uppercase;
        }
        input, textarea {
          width: 100%;
          padding: 10px 12px;
          ...
          background:#fff;
          outline: none;
          box-shadow: 0 4px 14px rgba(43,27,18,.06);
        ...
        .upload-inner{
          display:flex; align-items:center; gap:12px; color:#2b1b12;
        }
        .upload-icon{ font-size:20px; }
        .upload-text strong{ font-weight:700; }
        .upload-hint{ font-size:12px; color:#7a5c49; }
        ...
        .preview img{
          ...
          border-radius:12px; border:1px solid #eadacb;
          box-shadow:0 4px 14px rgba(43,27,18,.06);
          background:#fff;
        }
        .preview-actions{
        ...
        .btn {
          ...
          transition: filter .15s; border: 1px solid rgba(0,0,0,.08);
          box-shadow: 0 6px 14px rgba(0,0,0,.12);
        }
        .btn-primary { background: #8B4513; color:#fff; }
        .btn-primary:hover { filter: brightness(1.07); }
        .btn-ghost { background: #fff; color: #D97706; box-shadow: 0 6px 14px rgba(43,27,18,.12); }
